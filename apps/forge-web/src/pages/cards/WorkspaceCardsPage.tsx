import { ArrowRight, Braces, Download, Search } from "lucide-react";
...
export function WorkspaceCardsPage() {
  ...
  return <main className="card-detail-page"><section className="card-detail"><div className="card-detail-inner"><Link className="detail-breadcrumb" to="/cards">卡片库</Link><header className="detail-header"><div><div className="identity-line"><h1>{selected.name}</h1><code>{selected.id}</code></div><p>查看卡片效果、所需数据和检查结果。</p><div className="detail-meta"><span className="status-draft">{selected.mutable ? "草稿" : `v${selected.version}`}</span><span>{Object.keys(selected.samples).length} 个视图</span><span className="status-valid">{compiled.issues.length ? `${compiled.issues.length} 个问题` : "检查通过"}</span></div></div><div className="detail-actions"><Link className="button secondary" to="/playground"><Braces size={14} />用数据预览</Link><a className="button primary" href={serverPath(`/api/v1/cards/${encodeURIComponent(selected.reference)}/handoff`)}><Download size={14} />下载交付包</a></div></header><div className="detail-tabs" role="tablist">{[["preview", "预览"], ["contract", "数据结构"], ["validation", "检查结果"]].map(([key, label]) => <button key={key} type="button" className={tab === key ? "active" : ""} onClick={() => setQuery("tab", key)}>{label}</button>)}</div>
