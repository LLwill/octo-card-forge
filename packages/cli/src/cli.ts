...
// The dev server lives in the legacy application layer for now (Phase 3E
// migrates the CLI runtime; the HTTP server is a later phase). The root
// entry injects it so this package never depends back on the legacy root.
...
export type StartServer = (options: CliServerOptions) => Promise<void>;
...
try {
  if (command === "init") {
    ...
    await deps.startServer({ host, port, cardRoot, profile: profileSource });
  } else {
    usage();
