...
describe("Artifact consumer fixture (repo-free consumption)", () => {
  it("built artifact JSON is self-contained: can be parsed, verified, and inspected without Forge source", async () => {
    ...
    try {
      ...
      for (const [viewName, view] of Object.entries(artifact.views)) {
  ...
  it("verify CLI exits non-zero on tampered artifact and reports digest mismatch", async () => {
    ...
    try {
      ...
      const original = JSON.parse(await readFile(artifactPath, "utf8"));
      ...
      const { stdout, stderr } = await execFileAsync("node", [
        ...
        "--format",
        "json",
      ]).catch((err: any) => ({ stdout: err.stdout, stderr: err.stderr })) as any;
