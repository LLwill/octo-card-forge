...
description: Create, modify, validate, preview, and prepare versioned standard Adaptive Card packages in Octo Card Forge. Use when an Agent needs to build a new Octo card from requirements or a design, change card layout or interactions, define the backend-facing Card ViewModel JSON Schema, add views or samples, update an Octo Host Profile, or hand a card contract to a business backend.
---
...
1. Derive a lowercase namespaced ID, display name, initial view, and existing Host Profile from the requirement.
2. Run:
   ...
   Use `--view` or `--host-profile` only when the defaults are unsuitable.
...
7. Define stable action IDs, input guarantees, and local visibility state in `interactions.json`:
...
1. Read its manifest, contract, interactions, relevant templates, samples, and Host Profile capabilities.
...
4. Update the contract, samples, templates, interactions, and manifest together when the change crosses those boundaries.
...
## Handle Host Profiles

Change a Host Profile only for renderer-wide behavior shared by multiple cards. Never modify a released profile directory in place. Create a new version containing matching `host-config.json`, `adaptive-card.css`, and `capabilities.json`, then update card manifests to the new `<id>@<version>` reference.

Keep card-specific CSS and frontend markers out of Host Profiles. Production Octo Web receives Host Profile changes through its normal source-control and release process. Prefer controlled HTTPS assets; flag third-party icon URLs as prototype dependencies.
...
- Increment card `version` for every publishable package change.
  - Patch: presentation, template, or sample correction with unchanged compatible contract and interactions.
...
- Version Host Profiles independently; changing a card's Host Profile reference still requires a card version increment.
...
- Card ID and version; contract version; Host Profile reference.
...
- Remaining prototype assets, unresolved product choices, or required Octo Web Host Profile work.
